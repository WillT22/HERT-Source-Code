function [singleMatrix_whole,singleMatrix_inner,energy_beam,beam_number,hits_log,hits_log_inner,count_back_inner,count_back_whole,count_outer_inner,detector_energy_whole,detector_energy_inner,hits_detectors_whole,hits_detectors_inner] = oneEnergyEffDist(file_name,energy_channels,outer_limit,inputfolder,detector_threshold)
%Author:Yinbo Chen
%Date: 6/15/2021
...
singleMatrix_inner = zeros(length(energy_channels), 1);
singleMatrix_whole = zeros(length(energy_channels), 1);
hits_log = 0;
hits_log_inner = 0;

hits_detectors_whole= zeros(9,1);
hits_detectors_inner= zeros(17,1);

detector_energy_whole= zeros(9,1);
detector_energy_inner= zeros(17,1);
...
for i = 1:beam_number
    ...
    Whole_Detector_Energy = zeros(8,1);
    ...
    for j = 1:17
        ...
        if Detector_Energy(j)>0
            hits_detectors_inner(j) = hits_detectors_inner(j)+1;
        end
    end
    ...
    for k = 1:9
        ...
        % If the sum is below the detetor_threshold, it zeros out that
        % result
    ...
    detector_energy_whole = detector_energy_whole + [Whole_Detector_Energy];
    ...
    if Detector_Energy(17) > outer_limit
        count_back_whole = count_back_whole +1;
        count_back_inner = count_back_inner +1;
        WholeSum = 0;
        InnerSum = 0;
        reject_count=1;
        continue
    ...
    %For whole configuration, filters out values below detector threshold 
    for k = 1:8
        if Whole_Detector_Energy(k) < detector_threshold
            Whole_Detector_Energy(k) = 0;
        end
    end
    
    %For inner configuration, filters out detectors below threshold
    for L = 1:16
    ...
    WholeSum = sum(Whole_Detector_Energy);
    ...
    if WholeSum > 0  && Whole_Detector_Energy(1) > detector_threshold && reject_count == 0
